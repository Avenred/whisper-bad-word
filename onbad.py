...
class BadFinder:
    ...
    model       = "" # the model used by whisper
    ...
    def __init__(self,
        ...
        if use_custom: # if we're using a custom script we import it
            if custom_script_file != None:
            ...
            elif custom_script_str != None: # if the script provided is in a "str" and not a file on the disk we create a file and import it
                ...
                raise ValueError('No script was specified.') # if the "use_custom" variable is True but no input file or str is provided then raise an error
        else:
            pass
    ...
    def begin(self, sample_rate: int=44100, duration: int=10):
        record_thread     = Thread(target=self.record_and_save, args=(duration, sample_rate,)) # create all the threads
    ...
    def check(self): # the thread that checks the transcribed audio
        transcribed = self.transcribed # init vars 'cus I'm too lazy to add "self." to them
                    ...
                    for i in range(0, len(badwords)):
                        try:  # if bad word is found
                            if catch_reg: # if fuzzy mode
                                lower_transcribed.index(badwords[i]) # try to index the bad word
                            else: # if not fuzzy mode
                                lower_transcribed.index(' ' + badwords[i] + ' ') # try to index the bad word as it's own word
                            ...
                            """
                        except (ValueError, IndexError):  # if the index fails or if the non-fuzzy mode check fails
                            self.bad_word_action(True)
                        try:
